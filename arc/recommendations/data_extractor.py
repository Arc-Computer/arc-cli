...
class SimulationDataExtractor:
    ...
    def extract_minimal_context_for_testing(self, yaml_config: Dict[str, Any], test_scenarios: List[Dict[str, Any]]) -> SimulationContext:
        """Create minimal simulation context for testing purposes."""
        
        # Create mock performance metrics
        performance_metrics = {
            "reliability_score": 0.5,  # Assume moderate performance for testing
            "total_scenarios": len(test_scenarios),
            "success_count": 0,
            "failure_count": len(test_scenarios),  # Assume all failed for testing
            "run_id": "test_run",
            "timestamp": None
        }
        
        # Create failure data from test scenarios
        failure_data = []
        execution_trajectories = []
        
        for i, scenario in enumerate(test_scenarios):
            failure_data.append({
                "scenario_name": scenario.get("name", f"test_scenario_{i}"),
                "error_message": "Test failure for analysis",
                "task_prompt": scenario.get("task_prompt", ""),
                "expected_tools": scenario.get("expected_tools", []),
                "scenario_type": scenario.get("inferred_domain", "test"),
                "complexity": scenario.get("complexity_level", "medium"),
                "execution_time": 30
            })
            
            # Create mock trajectory
            execution_trajectories.append({
                "status": "error",
                "error": "Test failure for analysis",
                "full_trajectory": [],
                "execution_time_seconds": 30
            })
        
        return SimulationContext(
            original_yaml=yaml_config,
            scenario_data=test_scenarios,
            execution_trajectories=execution_trajectories,
            failure_data=failure_data,
            success_data=[],
            performance_metrics=performance_metrics
